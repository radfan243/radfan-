...
(function(){
  ...
  let lastBookSymbol='',bookBusy=false;
  ...
  async function book(){
    if(!window.state||bookBusy)return; const s=state.symbol||'BTCUSDT'; if(s!==lastBookSymbol){lastBookSymbol=s;$('orderBook').innerHTML='<div>جاري تحديث دفتر الطلبات…</div>'}
    bookBusy=true;
    try{
  ...
  function stats(){
    if(!window.state)return; const s=state.symbol||'BTCUSDT',t=(state.ticker||[]).find(x=>x.symbol===s); if(!t)return;
  ...
  async function enableNative(){
    ...
    try{
      ...
      if(plugin&&plugin.enable){await plugin.enable({symbol:state.symbol||'BTCUSDT',interval:state.interval||'15m'});$('alertText').textContent='تم تفعيل تنبيهات Android الخلفية. ستصل إشعارات الشراء والبيع حتى خارج التطبيق.'}
